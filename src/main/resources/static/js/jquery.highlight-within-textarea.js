	...
	HighlightWithinTextarea.prototype = {
		...
		getStringRanges: function(input, str) {
			...
			while (index = inputLower.indexOf(strLower, index), index !== -1) {
				if(index==0) { 
					if ((strLower.length == input.length) || !input[index + strLower.length].match(letters)) {
						ranges.push([index, index + strLower.length]);
					}
				} else if((index!=0) && (index + strLower.length == inputLower.length) && (!input[index-1].match(letters))) { 
					ranges.push([index, index + strLower.length]);
				} else if ((index!=0 && index + strLower.length != input.length && !input[index-1].match(letters) && !input[index + strLower.length].match(letters))) {
					ranges.push([index, index + strLower.length]);
				}
